...
プロパティ、属性、スタイル、クラスバインド、Plantillas補間、これらを利用し
PlantillasからDOMへ値を挿入する際、Angularは値のサニタイズとエスケープを行います。

_Plantillas文字列は実行可能コードと同じコンテキストです。_
Plantillas文字列（HTML、属性、およびバインディング式）は安全であると見なされており、
つまり攻撃者がPlantillasのコードを作成することはアプリケーション側で防止しなければなりません。
_Plantillasインジェクション_と呼ばれるこれらの脆弱性を避けるには
[オフラインPlantillasコンパイラ](guide/security#offline-template-compiler)を使用します。
...
次のPlantillasでは `htmlSnippet` の値を、まずPlantillas補間で要素内に展開し、
...
潜在的に安全でないメソッドが含まれています。同様に、DOMを操作する他のライブラリとやりとりする場合、Angularの補間と同じような自動サニタイズはありません。 DOMと直接対話するのではなく、可能であればAngularPlantillasを使用してください。
...
### オフラインPlantillasコンパイラを使う

オフライン・Plantillas・コンパイラはPlantillasインジェクションと呼ばれる脆弱性を確実に防止し
アプリケーションのパフォーマンスを大幅に向上させます。プロダクション環境ではオフラインPlantillasコンパイラを使い、
動的にPlantillasを生成しないようにしましょう。AngularはPlantillasコードを信頼するので、
Plantillas、特にユーザーデータを含むPlantillasを生成すると、Angularの組み込みの保護が回避されます。
AngularはPlantillas文字列を全面的に信頼するため、動的なPlantillas生成は常にXSSの危険性を有します。フォームを安全に動的に構築する方法については
[Dynamic Forms](guide/dynamic-form) のガイドを参照してください。

### サーバーサイドXSSへの対策

サーバーサイドで構築されたHTMLがXSS脆弱性を有することもあります。
これらをPlantillasとしてAngularへ注入することはアプリケーションに実行可能コードを注入することを意味し、
この場合アプリケーションは攻撃者によって完全に制御されてしまいます。
サーバーサイドでHTMLを構築する際も値のエスケープは確実に行ってください。
またサーバーサイドでAngularPlantillasを生成することは避けてください。
これらの処理はPlantillasインジェクションの脆弱性が発生する危険性を高めます。
...
次のPlantillasでは、ユーザーはYouTubeの動画IDを入力し、対応する動画を`<iframe>`に読み込むことができます。
